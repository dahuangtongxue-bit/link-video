...
function ImageCard({ shape }: { shape: ImageShape }) {
  ...
  const { status, imageUrl, error, videoModel, motion, prompt, w, h } = shape.props;
  ...
  async function handleMakeVideo() {
    ...
    try {
      const taskId = await submitVideo({ imageUrl, prompt: motionPrompt, model: videoModel });
    ...
    <HTMLContainer>
      <div style={cardShell(TOKENS.image, w, h)}>
        ...
        >
          ...
          {status === "error" && (
            <div style={{ padding: 16, fontSize: 12, color: TOKENS.error, textAlign: "center" }}>
              生成失败：{error}
            </div>
          )}
